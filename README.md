...
[![Last Commit](https://img.shields.io/github/last-commit/bohan-lj/Bohan)](https://github.com/bohan-lj/Bohan/commits/main)
...
| **⏱️ 定时器** | 支持周期定时器和一次性定时器，基于优先级队列 |
| **🔌 网络库** | 封装完整的 TCP 网络通信，支持连接管理和事件回调 |
| **🔒 线程安全** | 所有共享数据结构均有完善的锁机制保护 |
| **📦 跨平台** | Windows（IOCP）/ Linux（epoll）/ macOS（kqueue）全平台支持 |
| **📝 Protobuf 支持** | 内置 protobuf 协议支持 |
| **🔧 易扩展** | 模块化设计，支持快速业务定制 |
...
├─────────────────────────────────────────────────────────────────┤
│                          System Layer                            │
│            IOCP / epoll / kqueue / std::thread / mutex          │
└─────────────────────────────────────────────────────────────────┘
```
...
| `CLock` / `AutoLock` | RAII 锁封装 | 自动资源管理，避免死锁 |
| `Condition` | 条件变量封装 | 线程间同步等待 |
| `Singleton<T>` | 单例模式 | 全局管理器 |
| `Copyable` | 可拷贝标记基类 | 语义标记 |
| `BFile` | 文件操作封装 | 文件读写 |
| `InIConfigParser` | INI 配置解析 | 读取配置文件 |
...
### 2. Google Protobuf 协议

项目已内置 protobuf 头文件，支持使用 protobuf 定义服务接口，可直接在项目中使用。
...
| **原生事件驱动** | Windows IOCP / Linux epoll / macOS kqueue | 充分利用系统能力，高并发低延迟 |
| **零拷贝设计** | SimpleBuffer 缓冲区 | 减少内存拷贝，提升性能 |
| **跨平台兼容** | 统一接口封装底层差异 | 一次编写，多端运行 |
...
> 测试环境：Intel i7-10700K / Linux 5.15 / GCC 9.3  
> 以下为基准性能数据（仅供参考）：
